...
// TesterContextTestCase represents one element in the HELLOBYTE_TEST_CASES environment variable
...
// TesterContext holds all flags passed in via environment variables, or from the hellobyte.yml file
...
// 支持三种模式：
// 1. HELLOBYTE_TEST_CASES_JSON - 完整 JSON 格式（兼容 worker 调度）
// 2. HELLOBYTE_STAGE - 指定单个 stage slug（调试用）
// 3. 无环境变量 - 运行所有测试（默认行为）
//
// HELLOBYTE_REPOSITORY_DIR 默认为当前目录 "."
func GetTesterContext(env map[string]string, definition tester_definition.TesterDefinition) (TesterContext, error) {
	submissionDir, ok := env["HELLOBYTE_REPOSITORY_DIR"]
	if !ok {
		// 默认为当前目录
		submissionDir = "."
	}
	...
	skipAntiCheatValue, ok := env["HELLOBYTE_SKIP_ANTI_CHEAT"]
	...
	configPath := path.Join(submissionDir, "hellobyte.yml")
...
func parseTestCasesFromJSON(jsonStr string) ([]TesterContextTestCase, error) {
	...
	if err := json.Unmarshal([]byte(jsonStr), &testCases); err != nil {
		return nil, fmt.Errorf("failed to parse HELLOBYTE_TEST_CASES_JSON: %s", err)
	}

	for _, tc := range testCases {
		if tc.Slug == "" {
			return nil, fmt.Errorf("HELLOBYTE_TEST_CASES_JSON contains a test case with an empty slug")
		}
		if tc.TesterLogPrefix == "" {
			return nil, fmt.Errorf("HELLOBYTE_TEST_CASES_JSON contains a test case with an empty tester_log_prefix")
		}
		if tc.Title == "" {
			return nil, fmt.Errorf("HELLOBYTE_TEST_CASES_JSON contains a test case with an empty title")
		}
	}
...
func readFromYAML(configPath string) (yamlConfig, error) {
	...
	if err != nil {
		// hellobyte.yml is optional - return default config if not found
		if os.IsNotExist(err) {
			return yamlConfig{Debug: false}, nil
		}
		return yamlConfig{}, &internal.UserError{
			Message: fmt.Sprintf("Can't read hellobyte.yml file: %v", err),
		}
	}

	if err := yaml.Unmarshal(fileContents, c); err != nil {
		return yamlConfig{}, &internal.UserError{
			Message: fmt.Sprintf("Error parsing hellobyte.yml: %s", err),
		}
	}
