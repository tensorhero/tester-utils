...
数据流断链：`stage.yml → bootlab-schema syncer → DB stages.files_config (JSONB) → ❌ 无消费端`
...
- P1-B（数据驱动，后续）：Worker 通过 `BOOTLAB_FILES_CONFIG` 环境变量注入 JSON，
...
| Worker 侧 `BOOTLAB_FILES_CONFIG` 注入 | 需 Worker + API 协同改造，P1-B 阶段 |
